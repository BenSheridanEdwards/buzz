...
use crate::managed_agents::{save_managed_agents, BackendKind, RespondTo};
...
fn agent_readiness_draft(pubkey: &str) -> AgentReadinessDraft {
    AgentReadinessDraft {
...
fn mock_app_in_temp_data_dir() -> MockAppFixture {
...
fn persona_with_model(model: &str) -> AgentDefinition {
    let mut persona = persona_without_model();
    persona.model = Some(model.to_string());
    persona
}

fn persona_without_model() -> AgentDefinition {
    AgentDefinition {
        description: None,
...
#[test]
fn saved_readiness_evaluation_returns_structured_missing_requirements() {
    let mut record = agent_record();
    record.persona_id = None;
    record.runtime = Some("buzz-agent".to_string());
    record.agent_command = "buzz-agent".to_string();
    record.provider = Some("anthropic".to_string());
    record.model = Some("claude-opus-4-5".to_string());

    let evaluation = evaluate_agent_record_readiness(
        &record,
        &[],
    ...
    assert!(
        !evaluation.ready,
        "anthropic without ANTHROPIC_API_KEY must be NotReady"
    );
    assert_eq!(
...
#[test]
fn draft_readiness_override_can_be_ready_without_mutating_record() {
    let mut record = agent_record();
    record.persona_id = None;
    record.runtime = Some("buzz-agent".to_string());
    record.agent_command = "buzz-agent".to_string();
    ...
    let mut draft = agent_readiness_draft(&record.pubkey);
    draft.env_vars = Some(std::collections::BTreeMap::from([(
    ...
    assert!(
        evaluation.ready,
        "draft env var should satisfy the production readiness predicate"
    );
    assert!(
        original.env_vars.is_empty(),
        "draft evaluation must not require mutating the saved baseline"
    );
}

#[test]
fn draft_readiness_runtime_override_uses_runtime_catalog_env_keys() {
    let mut record = agent_record();
    record.persona_id = None;
    record.runtime = Some("claude".to_string());
    record.agent_command = "claude-agent-acp".to_string();
    record.provider = None;
    record.model = None;

    let mut draft = agent_readiness_draft(&record.pubkey);
    draft.runtime = Some(Some("buzz-agent".to_string()));
    draft.provider = Some(Some("anthropic".to_string()));
    draft.model = Some(Some("claude-opus-4-5".to_string()));
    draft.env_vars = Some(std::collections::BTreeMap::new());
    apply_draft_override(&mut record, draft);
    ...
    assert!(
        !evaluation.ready,
        "draft runtime should be evaluated as buzz-agent, not the saved Claude command"
    );
    assert!(
        evaluation.requirements.contains(&Requirement::EnvKey {
            key: "ANTHROPIC_API_KEY".to_string(),
        }),
        "missing requirements must come from the selected runtime/provider catalog; got {:?}",
        evaluation.requirements
    );
    assert!(
        !evaluation
            .requirements
            .iter()
            .any(|requirement| matches!(requirement, Requirement::CliLogin { .. })),
        "runtime override to buzz-agent must not run the saved Claude CLI-login readiness path"
    );
}

#[test]
fn draft_readiness_null_provider_clears_to_global_fallback() {
    let mut record = agent_record();
    record.persona_id = None;
    record.runtime = Some("buzz-agent".to_string());
    record.agent_command = "buzz-agent".to_string();
    ...
    let mut draft = agent_readiness_draft(&record.pubkey);
    draft.provider = Some(None);
    draft.model = Some(None);
    apply_draft_override(&mut record, draft);

    let evaluation = evaluate_agent_record_readiness(&record, &[], &global);

    assert!(
        evaluation.ready,
        "explicit null draft fields should clear saved values and fall through to global config"
    );
}

#[test]
fn draft_readiness_preserves_definition_authority_for_linked_instances() {
    let mut record = agent_record();
    ...
    let mut persona = persona_without_model();
    persona.provider = Some("openrouter".to_string());
    persona.model = Some("openrouter/model".to_string());
    persona.env_vars.insert(
        "OPENROUTER_API_KEY".to_string(),
        "sk-openrouter".to_string(),
    );
    ...
    assert!(
        evaluation.ready,
        "linked instances must still resolve provider/model from the persona, not draft instance fields"
    );
}

#[tokio::test]
async fn saved_readiness_command_reads_persisted_agent_and_returns_requirements() {
    let _guard = crate::managed_agents::lock_env_mutex();
    let fixture = mock_app_in_temp_data_dir();
    let mut record = agent_record();
    ...
    record.provider = Some("anthropic".to_string());
    record.model = Some("claude-opus-4-5".to_string());
    save_managed_agents(fixture.app.handle(), &[record.clone()]).expect("seed saved agent");

    let evaluation = evaluate_saved_agent_readiness_for_app(
        fixture.app.handle(),
        &fixture.app.state::<AppState>(),
        &record.pubkey,
    )
    .expect("command evaluates saved readiness");
...
#[tokio::test]
async fn draft_readiness_command_uses_overrides_without_writing_them() {
    let _guard = crate::managed_agents::lock_env_mutex();
    let fixture = mock_app_in_temp_data_dir();
    let mut record = agent_record();
    ...
    let evaluation = evaluate_draft_agent_readiness_for_app(
        fixture.app.handle(),
        &fixture.app.state::<AppState>(),
        draft,
    )
    .expect("command evaluates draft readiness");

    assert!(evaluation.ready);
    let saved = load_managed_agents(fixture.app.handle()).expect("reload saved agents");
    assert!(
        saved[0].env_vars.is_empty(),
        "draft command must not persist override env vars"
    );
}

#[tokio::test]
async fn draft_readiness_command_accepts_unsaved_create_baseline() {
    let _guard = crate::managed_agents::lock_env_mutex();
    let fixture = mock_app_in_temp_data_dir();
    let mut draft = agent_readiness_draft(&"c".repeat(64));
    draft.pubkey = None;
    draft.runtime = Some(Some("buzz-agent".to_string()));
    draft.provider = Some(Some("anthropic".to_string()));
    draft.model = Some(Some("claude-opus-4-5".to_string()));
    ...
    .expect("command evaluates create draft readiness");
    ...
    assert_eq!(
        ...
        "create drafts should use the runtime catalog without requiring a saved agent"
    );
}

#[test]
fn draft_readiness_custom_command_reports_missing_binary() {
    let mut record = agent_record();
    record.persona_id = None;
    let missing_command = "buzz-readiness-draft-missing-harness";

    let mut draft = agent_readiness_draft(&record.pubkey);
    draft.runtime = Some(None);
    draft.agent_command = Some(Some(missing_command.to_string()));
    apply_draft_override(&mut record, draft);
