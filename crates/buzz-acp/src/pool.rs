...
pub async fn run_prompt_task(
    ...
    // every section that actually travelled in the turn. Links carry no
    // model-visible text, so only text blocks are named.
    let prompt_text_blocks: Vec<&str> = prompt_blocks
        .iter()
        .filter_map(|block| match block {
            crate::acp::PromptBlock::Text(text) => Some(text.as_str()),
            crate::acp::PromptBlock::ResourceLink { .. } => None,
        })
        .collect();
    let mut prompt_sections_summary = prompt_block_labels(&prompt_text_blocks);
